...
Ind_Internal_Mod <-'Compulsive =~ an + 1*ocd + ts
...
Ind_Internal <- userGWAS(covstruc = LDSCoutput_CD, SNPs = split_sumstats, model = Ind_Internal_Mod, 
                           parallel = TRUE, cores = 6,
                           sub=c("mdd~SNP", "gad~SNP", "ptsd~SNP", "pau~SNP", "Compulsive~SNP", "Psychotic~SNP", "Neurodev~SNP"))
print("Ind Internal GWAS completed")

print("printing factor 1 SNPs")
CF1 <- Ind_Internal[[1]]
write.csv(CF1, file="./results_indep/Internal/F1_sumstats/NUMBER.csv", row.names=FALSE)

print("printing factor 2 SNPs")
CF2 <- Ind_Internal[[2]]
write.csv(CF2, file="./results_indep/Internal/F2_sumstats/NUMBER.csv", row.names=FALSE)

print("printing factor 3 SNPs")
CF3 <- Ind_Internal[[3]]
write.csv(CF3, file="./results_indep/Internal/F3_sumstats/NUMBER.csv", row.names=FALSE)

print("printing factor 4 SNPs")
CF4 <- Ind_Internal[[4]]
write.csv(CF4, file="./results_indep/Internal/F4_sumstats/NUMBER.csv", row.names=FALSE)
